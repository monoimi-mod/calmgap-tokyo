...
LOAD_COMPONENTS: tuple[Component, ...] = (
    ...
    Component(
        ...
        weight=0.9,
        sign=1,
        source="東京都環境局 自動車交通騒音 常時監視測定地点"
        "（令和元〜5年度の 5 年分・道路端の測定点）",
...
SOURCES: dict[str, Source] = {
    ...
    "tokyo_road_noise": Source(
        ...
        label="令和元〜5年度 自動車交通騒音 常時監視測定地点（東京都環境局）",
        ...
        # とは考えにくく、カタログへの登録が止まっただけに見える——が、
        # **そう見えることと、そう書いてあることは別**。ライセンス欄に
        # 前の値を持ち回ると、画面が確かめていないことを断言する
        # （出典リンクで一度やった壊れ方）。書いてある通りを書き、
        # 環境局への確認を `docs/issues.md` の宿題として残す。
        #
        # **配信物には測定点そのものを載せていない**（`web/public/data/` に
        # 出るのは内挿後のメッシュ値 `f_noise_db` だけ）。複製に当たり得るのは
        # リポジトリが追跡している `data/processed/noise.geojson` の側。
        license="東京都環境局サイトでの公開"
        "（二次利用の可否を環境局へ照会中。2026-08-04 に電話で問い合わせ済み）",
